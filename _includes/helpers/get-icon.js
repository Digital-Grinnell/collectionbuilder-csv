...
function getIcon(objectTemplate,objectFormat,svgType) {
    ...
    if (svgType == "thumb") {
        // svg sprite as thumb
        return '<svg class="bi text-body img-fluid" fill="currentColor" role="img"><title>' + iconTitle + '</title><use href="{{ "/assets/css/cb-icons.svg" | relative_url }}#' + iconId + '"/></svg>';
    } else if (svgType == "hidden") {
        // svg as sprite with aria-hidden
        return '<svg class="bi icon-sprite" aria-hidden="true"><use href="{{ "/assets/css/cb-icons.svg" | relative_url }}#' + iconId + '"/></svg>';
    } else {
        // svg as sprite
        return '<svg class="bi icon-sprite" aria-label="' + iconTitle + '"><use href="{{ "/assets/css/cb-icons.svg" | relative_url }}#' + iconId + '"/></svg>';
    }
}
